...
export const topics = pgTable(
  ...
  (table) => ({
    subjectUnitIdx: index("topics_subject_unit_idx").on(
      table.subjectId,
      table.unitId
    ),
...
export const sourcePapers = pgTable(
  ...
  (table) => ({
    subjectYearSessionIdx: index("papers_subject_year_session_idx").on(
      table.subjectId,
      table.year,
      table.session
    ),
...
export const questions = pgTable(
  ...
  (table) => ({
    subjectTopicIdx: index("questions_subject_topic_idx").on(
      table.subjectId,
      table.primaryTopicId
    ),
    difficultyIdx: index("questions_difficulty_idx").on(table.difficulty),
    sourceIdx: index("questions_source_idx").on(
      table.sourcePaperId,
...
export const answerOptions = pgTable(
  ...
  (table) => ({
    questionPartIdx: index("answer_options_question_part_idx").on(
...
export const questionAssets = pgTable(
  "question_assets",
  {
    ...
    questionId: uuid("question_id")
      .notNull()
      .references(() => questions.id, { onDelete: "cascade" }),
    questionPartId: uuid("question_part_id").references(
      () => questionParts.id,
      { onDelete: "cascade" }
    ),
...
export const userQuestionProgress = pgTable(
  ...
  (table) => ({
    userQuestionIdx: index("user_progress_user_question_idx").on(
      table.userId,
      table.questionId
    ),
