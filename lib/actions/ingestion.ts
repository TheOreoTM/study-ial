...
export type IngestionJob = Prisma.IngestionJobGetPayload<{}>;
export type IngestionJobInsert = Prisma.IngestionJobCreateInput;
export type SourcePaperInsert = Prisma.SourcePaperCreateInput;
...
export async function createSourcePaper(data: any) {
    const paper = await prisma.sourcePaper.create({
        data: {
            ...
            markingSchemeUrl: data.markingSchemeUrl,
...
export async function createIngestionJob(data: any) {
    const job = await prisma.ingestionJob.create({
        data: {
            ...
            processedQuestions: data.processedQuestions ?? 0,
            errorLog: data.errorLog ?? Prisma.JsonNull,
            rawGeminiOutput: data.rawGeminiOutput ?? Prisma.JsonNull,
 ...
 */
export async function updateIngestionJobProgress(jobId: string, data: Partial<IngestionJob>) {
    const updated = await prisma.ingestionJob.update({
        where: { id: jobId },
        data: {
            ...data,
            errorLog: data.errorLog ?? undefined,
            rawGeminiOutput: data.rawGeminiOutput ?? undefined,
        },
    });
...
export async function getIngestionStatistics() {
    const [totalJobs, completedJobs, failedJobs, pendingJobs, processingJobs, totalQuestionsProcessed] =
        await Promise.all([
            ...
            prisma.ingestionJob.count({ where: { status: "processing" } }), // Assuming 'processing' is a valid status string
    ...
    return {
        ...
        pendingJobs,
        processingJobs,
        totalQuestionsProcessed: totalQuestionsProcessed._sum.processedQuestions || 0,
        averageQuestionsPerJob:
            completedJobs > 0 ? (totalQuestionsProcessed._sum.processedQuestions || 0) / completedJobs : 0,
    };
}
